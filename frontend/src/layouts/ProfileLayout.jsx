        ...
        <div className="w-full max-w-md md:max-w-lg bg-white p-4 sm:p-6 md:p-8 flex flex-col text-center rounded-2xl sm:rounded-3xl text-base sm:text-lg shadow-xl sm:shadow-2xl mx-auto">
            ...
            <form className="w-full h-full px-1 sm:px-3 md:px-6">
                ...
                <div className="flex flex-row my-4 sm:my-5 min-w-full relative">
                    ...
                    <input className="border-b-2 w-full pl-10 pr-4 py-2.5 text-sm sm:text-base outline-none focus:border-violet-600 transition-colors" type="text" placeholder="username" />
                </div>
                {errors.username && <span className="text-red-500 text-sm">{errors.username?.message}</span>}
                {/* Password - Input */}
                <div className="flex flex-row my-4 sm:my-5 min-w-full relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-2/3 h-5 w-5 text-black/60" />
                    <input className="border-b-2 w-full pl-10 pr-4 py-2.5 text-sm sm:text-base outline-none focus:border-violet-600 transition-colors" type="password" placeholder="password" />
                </div>
